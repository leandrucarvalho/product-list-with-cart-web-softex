...
export class Cart {
  ...
  static renderCart() {
    ...
    const quantityTotal = cartHtml.firstElementChild?.firstElementChild;

    if (!quantityTotal) return;

    quantityTotal.innerHTML = this._products.length.toString();

    const cartItems = cartHtml.querySelector(".cart-items");
    if (!cartItems) return;

    for (const product of this._products) {
      cartItems.innerHTML = `
      <h2 class="font-bold text-[#f44336] text-xl">
          Your Cart (<span id="cart-count">7</span>)
      </h2>
        <div id="cart-items">
          <div class="cart-item flex flex-col">
            ...
            <div class="item-info text-xs">
              <span class="quantity text-[#f44336] font-bold">2x</span>
              <span class="unit-price text-[#c9aea6]">$10.00</span>
              <span class="total-price text-[#ad8985]">$20.00</span>
      ...
      cartHtml.appendChild(cartItems);
    }
  }
}
